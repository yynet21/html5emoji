function FractalRoot(ctx,pos,sides,angle)  {						
  this.ctx=ctx;
...
FractalRoot.prototype.update=function() {
  ...
  this.strutFactor = noise(this.strutNoise) * 1.3;
    for (var i = 0; i<this.sides; i++) {		
    var x = this.pos.x +(400 * cos(this.angle+frameCount*0.001+i/this.sides));
    var y = this.pos.y +(400 * sin(this.angle+frameCount*0.001+i/this.sides));
    this.point[i] = {x:x,y:y};		
  }
  this.obj=[];
  this.obj[0]={out:this.point,mid:null,proj:null,lev:0,num:0};
 ...
 FractalRoot.prototype.draw =function() {//値の変更
     ...
     for (var i=0;i<this.obj.length;i++){
       ...
       for (var j = 0; j < this.obj[i].mid.length; j++) {
         this.ctx.moveTo(this.obj[i].mid[j].x,this.obj[i].mid[j].x);
         this.ctx.moveTo(this.obj[i].proj[j],this.obj[i].mid[j].y);
         //k++;
       }
     }
     this.ctx.stroke();
